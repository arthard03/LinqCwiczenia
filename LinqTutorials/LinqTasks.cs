    ...
    namespace LinqTutorials
    {
        public static class LinqTasks
        {
            ...
            {
                double salaryMax = Emps.Max(emp => emp.Salary);
                IEnumerable<Emp> result = Emps.Where(emp => emp.Salary == salaryMax);
                return result;
            }
            ...
            {
                var oddOccurrences = arr.GroupBy(x => x)
                    .Where(g => g.Count() % 2 == 1)
                    .Select(g => g.Key);

                int result = oddOccurrences.Single();
            ...
            public static IEnumerable<Dept> Task14()
            {
                var result = from dept in Depts
                    join empGroup in (from emp in Emps
                            group emp by emp.Deptno
                            into g
                            select new { Deptno = g.Key, Count = g.Count() })
                        on dept.Deptno equals empGroup.Deptno into empGroups
                    from empGroup in empGroups.DefaultIfEmpty()
                    where empGroup == null || empGroup.Count == 5
                    orderby dept.Dname
                    select dept;
            ...
            {
                var result = Emps
                    .Where(emp => emp.Job.ToLower().Contains('a') || emp.Job.ToUpper().Contains('A'))
